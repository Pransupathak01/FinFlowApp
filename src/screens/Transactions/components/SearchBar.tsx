...
export function SearchBar({ onSearch, filterCount, onFilterPress }: Props) {
  ...
  return (
    <View style={s.wrapper}>
      {/* Search input */}
      <Animated.View style={[s.inputWrap, { transform: [{ scale }] }]}>
        <MaterialCommunityIcons name="magnify" size={18} color="#4A5568" style={s.searchIcon} />
        <TextInput
          ref={inputRef}
          style={s.input}
        ...
        {value.length > 0 && (
          <TouchableOpacity onPress={handleClear} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <MaterialCommunityIcons name="close-circle" size={16} color="#4A5568" />
          </TouchableOpacity>
        )}
      </Animated.View>

      {/* Filter button */}
      <TouchableOpacity style={s.filterBtn} onPress={onFilterPress} activeOpacity={0.8}>
        <MaterialCommunityIcons name="filter-variant" size={20} color={filterCount > 0 ? '#4F8EF7' : '#7A8499'} />
        {filterCount > 0 && (
          <View style={s.badge}>
